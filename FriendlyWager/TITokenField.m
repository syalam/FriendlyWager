...
- (void)setup {
	...
	contentView = [[UIView alloc] initWithFrame:CGRectMake(0, tokenFieldBottom, self.bounds.size.width,
														   self.bounds.size.height - tokenFieldBottom)];
...
- (void)updateContentSize {
	[self setContentSize:CGSizeMake(self.bounds.size.width, CGRectGetMaxY(contentView.frame))];
}
...
- (void)tokenFieldFrameWillChange:(TITokenField *)field {
	...
	[contentView setFrame:((CGRect){{contentView.frame.origin.x, (tokenFieldBottom
...
- (NSString *)searchResultSubtitleForRepresentedObject:(id)object {
	...
	if ([object objectForKey:@"city"]) {
        NSString *cityState = [object objectForKey:@"city"];
        if ([object objectForKey:@"state"]) {
            cityState = [NSString stringWithFormat:@"%@, %@", cityState, [object objectForKey:@"state"]];
        }
        return [cityState capitalizedString];
    }
	return nil;
}
